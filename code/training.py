# # library
import os

import random
import json
import pickle
import numpy as np

import nltk
from nltk.stem import WordNetLemmatizer

from tensorflow.keras.models import Sequential
from tensorflow.keras.layers import Dense, Dropout
from tensorflow.keras.optimizers import SGD


# directory check
def check_dir(name, our_path=os.path.dirname(os.path.abspath(__file__))) -> str:
    """
    :param name: dir name
    :param our_path: path to our dir
    :return: path to the created directory
    """

    arr = os.listdir(path='.')
    return_path = '{}/{}'.format(our_path, name)
    if name not in arr:
        os.mkdir(return_path)

    return return_path


# if it gives an error
nltk.download('punkt')
nltk.download('wordnet')


def upfate_intense() -> str:
    lemmatizer = WordNetLemmatizer()

    tmp_intents = json.loads(open('src/intense.json').read())

    for language in tmp_intents.keys():
        """
        we go through all our languages
        """

        path = check_dir('src')

        intents = tmp_intents[language]

        words = []
        classes = []
        documents = []
        ignore_letters = ['?', '!', ',', '.']

        for intent in intents['intents']:
            """
            go through all tags and take patterns from there
            """
            for pattern in intent['patterns']:
                """
                tokenization is the process of breaking up text into text units, such as words or sentences
                """
                if language == 'ru':
                    word_list = nltk.word_tokenize(pattern, language='russian')
                else:
                    word_list = nltk.word_tokenize(pattern)
                words.extend(word_list)
                documents.append((word_list, intent['tag']))
                if intent['tag'] not in classes:
                    classes.append(intent['tag'])

        words = [lemmatizer.lemmatize(word) for word in words if word not in ignore_letters]
        words = sorted(set(words))

        classes = sorted(set(classes))

        pickle.dump(words, open(f'{path}/words_{language}.pkl', 'wb'))
        pickle.dump(classes, open(f'{path}/classes_{language}.pkl', 'wb'))

        training = []
        output_empty = [0] * len(classes)

        for document in documents:
            bag = []
            word_patterns = document[0]
            word_patterns = [lemmatizer.lemmatize(word.lower()) for word in word_patterns]
            for word in words:
                bag.append(1) if word in word_patterns else bag.append(0)

            output_row = list(output_empty)
            output_row[classes.index(document[1])] = 1
            training.append([bag, output_row])

        random.shuffle(training)
        training = np.array(training)

        train_x = list(training[:, 0])
        train_y = list(training[:, 1])

        model = Sequential()
        model.add(Dense(128, input_shape=(len(train_x[0]),), activation='relu'))
        model.add(Dropout(0.5))
        model.add(Dense(64, activation='relu'))
        model.add(Dropout(0.5))
        model.add(Dense(len(train_y[0]), activation='softmax'))

        sgd = SGD(lr=0.01, decay=1e-6, momentum=0.9, nesterov=True)
        model.compile(loss='categorical_crossentropy', optimizer=sgd, metrics=['accuracy'])

        hist = model.fit(np.array(train_x), np.array(train_y), epochs=200, batch_size=5, verbose=1)
        model.save(f'{path}/chatbotmodel_{language}.h5', hist)

    return 'Done!'
